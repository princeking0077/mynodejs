...
export default function GPATContentManager() {
    ...
    const fetchSubjects = async () => {
        try {
            ...
            if (r.ok) {
                ...
                // Filter only GPAT subjects (those containing 'gpat' or specific GPAT IDs)
                const gpatSubjects = subjects.filter(s =>
                    s.id.toLowerCase().includes('gpat') ||
                    ['pharmacology', 'pharmaceutics', 'pharmaceutical-chemistry', 'pharmaceutical-analysis',
                     'pharmacognosy', 'biochemistry', 'microbiology', 'biotechnology',
                     'pathophysiology', 'clinical-pharmacy', 'hospital-community-pharmacy',
                     'pharmaceutical-jurisprudence', 'pharmaceutical-engineering',
                     'biostatistics-research-methodology', 'general-awareness'].includes(s.id)
                );
                setAllSubjects(gpatSubjects);
            }
    ...
    return (
        <AdminLayout title="GPAT Content Manager">
            ...
            <div style={{ maxWidth: 1100 }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                    <select value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} style={{ ...inputStyle, flex: '0 0 auto', width: 320 }}>
                        <option value="">— Select GPAT Subject —</option>
                ...
                {/* Stats */}
                {selectedSubject && (
                    <div style={{ padding: '1rem', background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 10, marginBottom: '1.5rem' }}>
                ...
                {loading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '3rem', color: 'rgba(255,255,255,0.3)' }}>
                        No topics found. {selectedSubject && <button onClick={openNew} style={{ color: '#8b5cf6', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Add your first topic?</button>}
                    </div>
                ) : !selectedSubject && !search ? (
                    <div style={{ textAlign: 'center', padding: '3rem', color: 'rgba(255,255,255,0.3)' }}>
                        <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>🎓</div>
                        <div style={{ fontSize: '1.2rem', fontWeight: 600, marginBottom: '0.5rem', color: 'white' }}>GPAT Content Manager</div>
                        <div>Select a subject above to view or add topics</div>
                        <div style={{ marginTop: '1rem', fontSize: '0.85rem', color: 'rgba(255,255,255,0.4)' }}>{allSubjects.length} GPAT subjects available</div>
                    </div>
                ) : null}
                        ...
                        <div key={t.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.25rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12 }}>
                            <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.78rem' }}>
                                    {t.subject_id} {t.unit_number ? `· Module ${t.unit_number}` : ''} {t.slug ? `· /${t.slug}` : ''}
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: '0.5rem', marginLeft: '1rem', flexShrink: 0 }}>
                                <button onClick={() => openEdit(t)} style={{ ...actionBtn, color: '#8b5cf6', background: 'rgba(139,92,246,0.1)' }}><Pencil size={14} /></button>
                ...
                {/* Form Modal - Same as B.Pharm but with GPAT branding */}
                {showForm && (
                        ...
                        <div style={{ background: '#0d0d1f', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: '2rem', width: '100%', maxWidth: 720, position: 'relative' }}>
                            ...
                            <div className="admin-form-two-col">
                                <div style={{ gridColumn: '1 / -1', marginBottom: '1rem' }}>
                                    <label style={labelStyle}>GPAT Subject *</label>
                                ...
                                <div style={{ gridColumn: '1 / -1', marginBottom: '1rem' }}>
                                    ...
                                    <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} style={inputStyle} placeholder="e.g. Adrenergic Agonists" />
                                </div>
                            </div>
                            ...
                            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '1rem', marginTop: '0.5rem' }}>
                                ...
                                <div className="admin-form-two-col">
                                    ...
                                    <div style={{ marginBottom: '1rem' }}>
                                        ...
                                        <input value={form.primaryKeyword} onChange={e => setForm(f => ({ ...f, primaryKeyword: e.target.value }))} style={inputStyle} placeholder="gpat pharmacology notes" />
                                    </div>
                                </div>
                                <div style={{ marginBottom: '1rem' }}>
                            ...
                            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'space-between', alignItems: 'center' }}>
                                <button
                                    ...
                                    style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem 1.5rem', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 10, color: '#10b981', cursor: 'pointer', fontWeight: 600 }}
                                >
                                    <Eye size={15} /> Preview
                                </button>
                                <div style={{ display: 'flex', gap: '0.75rem' }}>
                                    ...
                                    <button onClick={save} disabled={saving} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem 1.5rem', background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer', fontWeight: 700, opacity: saving ? 0.7 : 1 }}>
